async function init(t, e, l = '450px', s = '550px', domain = 'https://owllee.io') {
  try {
    ...
    document.addEventListener('mousemove', (event) => {
      if (isDragging) {
        r.style.right = `${window.innerWidth - event.clientX - offsetX}px`;
        r.style.bottom = `${window.innerHeight - event.clientY - offsetY}px`;
      }
    });
    ...
    document.addEventListener('mouseup', () => {
      ...
      r.style.left = Math.max(20, Math.min(rect.left, window.innerWidth - rect.width - 20)) + 'px';
      r.style.top = (window.innerHeight - rect.height - 20) + 'px';


      // Xóa lớp 'grabbing' để trở lại kiểu con trỏ mặc định
